...
Layout* BattleControl::getBattleUI()
{
	return _battleUI;
}
...
void BattleControl::setBattleUI(Layout *battleui)
{
	_battleUI = battleui;
}
...
void BattleControl::load()
{
	...
	_battleUI->addNode(_cardTouch, -1);
}
		...
		if (_heros)
		{
			...
			CCARRAY_FOREACH(_heros, hero)
			{
				...
				if (heroCard)
				{
					...
					Layout *layer = dynamic_cast<Layout*>(_battleUI->getChildByName(ui_name));
					...
					_battleUI->addNode(node, node->getZOrder());
				}
			}
		}
		step++;
...
void BattleControl::addReadyCard(Card* card, HeroCard *hero)
{
	if (card && hero)
	{
		_battleUI->addNode(card->getNode(), ORDER_CARD);
